...
var simple_storage = require(path.join(__dirname, '/storage/simple_storage.js'))
var ConsoleLogger = require(path.join(__dirname, '/console_logger.js'))
...
function Botkit (configuration) {
  ...
  function Conversation (task, message) {
    ...
    this.capture = function (response) {
      var captureKey = this.sent[this.sent.length - 1].text
      ...
      if (this.capture_options.key !== undefined) {
        captureKey = this.capture_options.key
      }
      ...
      if (this.capture_options.multiple) {
        if (!this.responses[captureKey]) {
          this.responses[captureKey] = []
        }
        this.responses[captureKey].push(response)
      } else {
        this.responses[captureKey] = response
      }
    }
    ...
    this.addQuestion = function (message, cb, captureOptions, thread) {
      if (typeof (message) === 'string') {
        message = {
          text: message,
          channel: this.source_message.channel
      ...
      if (captureOptions) {
        message.capture_options = captureOptions
      }
    ...
    this.ask = function (message, cb, captureOptions) {
      this.addQuestion(message, cb, captureOptions, this.thread || 'default')
    }
    ...
    this.combineMessages = function (messages) {
      ...
      if (messages.length > 1) {
        var txt = []
        var lastUser = null
        var multiUsers = false
        lastUser = messages[0].user
        for (var x = 0; x < messages.length; x++) {
          if (messages[x].user !== lastUser) {
            multiUsers = true
          }
        }
        lastUser = ''
        for (let x = 0; x < messages.length; x++) {
          if (multiUsers && messages[x].user !== lastUser) {
            lastUser = messages[x].user
            if (txt.length) {
              txt.push('')
            }
          ...
          if (this.messages.length) {
            ...
            if (typeof (this.messages[0].timestamp) === 'undefined' ||
              ...
              if (message.text || message.attachments || message.attachment) {
                ...
                this.task.bot.reply(this.source_message, outbound, function (err, sentMessage) {
                  if (err) {
                    ...
                    that.sent[that.sent.length - 1].sent = true
                    that.sent[that.sent.length - 1].api_response = sentMessage

                    // if sending via slack's web api, there is no further confirmation
                    // so we can mark the message delivered
                    if (that.task.bot.type === 'slack' && sentMessage && sentMessage.ts) {
  ...
  botkit.storage = {
    teams: {
      get: function (teamId, cb) {
        cb(null, botkit.memory_store.teams[teamId])
      },
    ...
    users: {
      get: function (userId, cb) {
        cb(null, botkit.memory_store.users[userId])
      },
    ...
    channels: {
      get: function (channelId, cb) {
        cb(null, botkit.memory_store.channels[channelId])
      },
   ...
   */
  botkit.changeEars = function (newTest) {
    botkit.hears_test = newTest
  }

  botkit.hears = function (keywords, events, middlewareOrCb, cb) {
    // the third parameter is EITHER a callback handler
    // or a middleware function that redefines how the hear works
    var testFunction = botkit.hears_test
    if (cb) {
      testFunction = middlewareOrCb
    } else {
      cb = middlewareOrCb
    }
    ...
    for (var e = 0; e < events.length; e++) {
      (function (keywords, testFunction) {
        botkit.on(events[e], function (bot, message) {
          if (testFunction && testFunction(keywords, message)) {
  ...
  botkit.startTicking = function (tickTime) {
